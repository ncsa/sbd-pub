...
std::vector<size_t> DetFromAlphaBeta(const std::vector<size_t> &A,
                                     ...
                                     const size_t bit_length, const size_t L) {
...
void DetFromAlphaBeta(const std::vector<size_t> &A,
                      const std::vector<size_t> &B, const size_t bit_length,
                      const size_t L, std::vector<size_t> &D) {
  int fsize = L / bit_length;
  int half = bit_length / 2;
  int extra = L - fsize * bit_length;
  int case1 = (extra > 0) && (extra <= half);
...
void setocc(std::vector<size_t> &dets, const size_t bit_length, int x, bool y) {
  ...
  if (y) {
    dets[block] |= (size_t(1) << bit);
  } else {
    dets[block] &= ~(size_t(1) << bit);
  }
}

inline bool getocc(const std::vector<size_t> &det, const size_t bit_length,
                   int x) {
...
inline int bitcount(const std::vector<size_t> &det, const size_t bit_length,
                    const size_t L) {
  int count = 0;
  size_t full_words = L / bit_length;
  size_t remaining_bits = L % bit_length;
  for (size_t i = 0; i < full_words; ++i) {
    count += __builtin_popcountll(det[i]);
  }
  if (remaining_bits > 0) {
...
int getClosed(const std::vector<size_t> &det, const size_t bit_length,
              const size_t L, std::vector<int> &closed) {
  int cindex = 0;
...
int getOpenClosed(const std::vector<size_t> &det, const size_t bit_length,
                  const size_t L, std::vector<int> &open,
...
void parity(const std::vector<size_t> &dets, const size_t bit_length,
...
std::vector<size_t> getAlpha(const std::vector<size_t> &det, size_t bit_length,
                             size_t norb) {
...
std::vector<size_t> getBeta(const std::vector<size_t> &det, size_t bit_length,
                            size_t norb) {
...
int difference(const std::vector<size_t> &a, const std::vector<size_t> &b,
...
void OrbitalDifference(const std::vector<size_t> &a,
                       const std::vector<size_t> &b, size_t bit_length,
                       size_t L, std::vector<int> &x, std::vector<int> &y) {
  x.clear();
  y.clear();
...
ElemT ZeroExcite(const std::vector<size_t> &det, const size_t bit_length,
                 const size_t L, const ElemT &I0, const oneInt<ElemT> &I1,
...
template <typename ElemT>
ElemT OneExcite(const std::vector<size_t> &det, const size_t bit_length, int &i,
                int &a, const oneInt<ElemT> &I1, const twoInt<ElemT> &I2) {
  double sgn = 1.0;
...
template <typename ElemT>
ElemT TwoExcite(const std::vector<size_t> &det, const size_t bit_length, int &i,
                int &j, int &a, int &b, const oneInt<ElemT> &I1,
                const twoInt<ElemT> &I2) {
  double sgn = 1.0;
  int I = std::min(i, j);
  int J = std::max(i, j);
...
template <typename ElemT>
ElemT Hij(const std::vector<size_t> &DetA, const std::vector<size_t> &DetB,
          const size_t &bit_length, const size_t &L, const ElemT &I0,
          const oneInt<ElemT> &I1, const twoInt<ElemT> &I2, size_t &orbDiff) {
  std::vector<int> c;
  std::vector<int> d;
  size_t nc = 0;
  size_t nd = 0;
  ...
  if (nc == 0) {
    ...
    orbDiff =
        static_cast<size_t>(c[1] * L * L * L + d[1] * L * L + c[0] * L + d[0]);
...
template <typename ElemT>
ElemT Hij(const std::vector<size_t> &DetA, const std::vector<size_t> &DetB,
          const size_t &bit_length, const size_t &L, std::vector<int> &c,
          std::vector<int> &d, const ElemT &I0, const oneInt<ElemT> &I1,
          const twoInt<ElemT> &I2, size_t &orbDiff) {
  size_t nc = 0;
  size_t nd = 0;
...
void ShuffleDet(std::vector<std::vector<size_t>> &det, unsigned int seed) {
  if (det.size() <= 1)
    return;
  std::mt19937 g(seed);
