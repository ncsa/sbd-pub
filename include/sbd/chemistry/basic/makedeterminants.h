...
void SetupDeterminants(const std::vector<std::vector<size_t>> &AlphaDet,
                       ...
                       std::vector<std::vector<size_t>> &Det, MPI_Comm comm) {
  ...
  for (size_t na = na_begin; na < na_end; na++) {
    for (size_t nb = 0; nb < NcB; nb++) {
      Det[NcB * (na - na_begin) + nb] =
          DetFromAlphaBeta(AlphaDet[na], AlphaDet[nb], bit_length, L);
    }
  }
}
...
void SetupDeterminants(const std::vector<std::vector<size_t>> &AlphaDet,
  ...
  for (size_t na = na_begin; na < na_end; na++) {
    for (size_t nb = 0; nb < NcB; nb++) {
      Det[NcB * (na - na_begin) + nb] =
          DetFromAlphaBeta(AlphaDet[na], BetaDet[nb], bit_length, L);
    }
  }
}

std::vector<std::vector<size_t>> DecodeAlphaDets(const std::string &filename,
                                                 size_t norb) {
  // open binary file
...
void LoadFromAlphaDets(const std::string &filename,
                       std::vector<std::vector<size_t>> &config, size_t norb,
                       size_t bit_length, MPI_Comm comm) {
...
void LoadAlphaDets(const std::string &adetfile,
                   std::vector<std::vector<size_t>> &adet, size_t bit_length,
